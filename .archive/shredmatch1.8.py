...
def search_uuid_in_logs(uuid, log_files, logs_path):
    ...
    for log_file in log_files:
        ...
        print(f"Searching UUID {uuid} in log: {log_file}")
        with open(log_path, "r") as log:
            for line in log:
                ...
                if match:
                    ...
                    print(f"Found matching line in {log_file}: {line.strip()}")
                    return import_folder, name

    print(f"No match found for UUID {uuid} in the provided log files.")
    return None, None
