...
def search_customer_info_in_logs(uuid, log_files, logs_path):
    """Search for UUID in the specified log files to extract customer information."""
    import_folder = None
    name = None
    proper_name = None
    customer_email = None
    ...
    customer_pattern = r"(added new customer|updating customer).*" + re.escape(uuid) + r".* ([\w.%-]+@[a-zA-Z.-]+)"
    import_pattern = r"starting reading " + re.escape(uuid) + r" .*(/import-[\w-]+)"

    for delta, log_file in enumerate(log_files, start=-2):
        ...
        with open(log_path, "r") as log:
            for line in log:
                if not proper_name and re.search(customer_pattern, line, re.IGNORECASE):
                    match = re.search(customer_pattern, line, re.IGNORECASE)
                    proper_name = match.group(2)
                    customer_email = match.group(3)
                    matched_log_file = log_file
                    delta_offset = delta

                if not import_folder and re.search(import_pattern, line, re.IGNORECASE):
                ...
                if proper_name and import_folder:
                    return {
...
def display_results(results, source_path, shredlogs_path):
    ...
    for group in results:
        ...
        if group['customer_info']:
            customer_info = group["customer_info"]
            if customer_info["import_folder"] and customer_info["name"]:
                print(f"    import folder: {customer_info['import_folder']}")
                print(f"    name: {customer_info['name']}")
            print(f"    proper name: {customer_info.get('proper_name', 'None')}")
            print(f"    customer_email: {customer_info.get('customer_email', 'None')}")
            print(f"    log_file: {customer_info.get('matched_log_file', 'None')}")
            print(f"    day_track: {customer_info.get('delta_offset', 'None')}")

            day_counts[str(customer_info.get("delta_offset", "0"))] += 1
        else:
    ...
    for file in source_files:
        if "photo-" in file.lower():
            ...
            if uuid and timestamp:
                ...
                # Only process the relevant logs for the current UUID
