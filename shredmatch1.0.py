...
def find_related_files(source_files, uuid):
    ...
    return [f for f in source_files if re.search(uuid, os.path.basename(f), re.IGNORECASE)]
...
def display_results(results):
    """Display matched results in a readable format."""
    print("\nMatched Results:")
    for group in results:
        print("\n---")
        print(f"Photo File: {group['photo_file']}")
        print(f"Related Files: {', '.join(group['related_files'])}")
        print(f"Log File: {group['log_file'] if group['log_file'] else 'No matching log file found'}")
    print("\n")
...
def main():
    ...
    for file in source_files:
        if "photo-" in os.path.basename(file).lower():
