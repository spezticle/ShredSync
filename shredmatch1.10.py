...
def search_customer_info(uuid, matching_logs, logs_path):
    """Search for customer info in matching logs."""
    proper_name = None
    customer_email = None
    pattern = r"(added new customer|updating customer).*" + re.escape(uuid) + r".* ([\w.%-]+@[a-zA-Z.-]+)"
    ...
    for log_file in matching_logs:
        ...
        with open(log_path, "r") as log:
            for line in log:
                match = re.search(pattern, line, re.IGNORECASE)
                if match:
                    proper_name = match.group(1)
                    customer_email = match.group(2)
    ...
    for file in source_files:
        if "photo-" in file.lower():
            ...
            if uuid and timestamp:
                ...
                if matching_logs:
                    proper_name, customer_email = search_customer_info(uuid, matching_logs, shredlogs)
                else:
